...
insert = r'''            // F3 0F 5A: CVTSS2SD, F2 0F 5A: CVTSD2SS.
            // Legacy scalar conversions replace only the low destination lane.
...
test_insert = r'''static void test_scalar_precision_conv(void) {
    ...
    memcpy(memory, ss2sd, sizeof(ss2sd));
    struct cpu_state cpu = fresh_cpu(&mmu);
    float sf = 3.25f;
    double expected_d = 3.25;
    memcpy(&cpu.xmm[0].u32[0], &sf, sizeof(sf));
    ...
    uint64_t raw_d;
    memcpy(&raw_d, &expected_d, sizeof(raw_d));
    assert(cpu.xmm[1].u64[0] == raw_d);
    ...
    memcpy(memory, sd2ss, sizeof(sd2ss));
    cpu = fresh_cpu(&mmu);
    double sd = -2.5;
    float expected_f = -2.5f;
    memcpy(&cpu.xmm[0].u64[0], &sd, sizeof(sd));
    cpu.xmm[1].u32[1] = 0xaabbccddU;
    cpu.xmm[1].u64[1] = 0x8877665544332211ULL;
    interrupt = cpu_run_to_interrupt(&cpu, NULL);
    assert(interrupt == INT_SYSCALL && cpu.x86_last_syscall == 60);
    uint32_t raw_f;
    memcpy(&raw_f, &expected_f, sizeof(raw_f));
    assert(cpu.xmm[1].u32[0] == raw_f);
    assert(cpu.xmm[1].u32[1] == 0xaabbccddU);
    assert(cpu.xmm[1].u64[1] == 0x8877665544332211ULL);
...
print("patched x86_64 scalar precision conversions")
