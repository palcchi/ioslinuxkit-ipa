...
Add the smallest fixture that fails before the fix and passes after it. Low-level examples live under `tests/arm64/atomics/`, `tests/arm64/loadstore/` and `tests/arm64/signals/`; broader rows are generated by `tests/arm64/runtime-coverage.sh`.
...
Commit source, its regression fixture and the relevant documentation in the same tranche. Push only after the local branch is based on the intended remote tip.
